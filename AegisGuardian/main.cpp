...
namespace fs {
    ...
    long long file_size(const string& path) {
        struct stat buffer;
        if (stat(path.c_str(), &buffer) == 0) return buffer.st_size;
        return 0;
    }
    void copy_file(const string& src, const string& dest) {
        CopyFileA(src.c_str(), dest.c_str(), FALSE);
    }
    void rename(const string& oldp, const string& newp) {
        ::rename(oldp.c_str(), newp.c_str());
    }
    ...
    void remove_all(const string& path) {
        ...
        if (hFind != INVALID_HANDLE_VALUE) {
            do {
                string name = findData.cFileName;
                if (name != "." && name != "..") ::remove((path + "/" + name).c_str());
            } while (FindNextFileA(hFind, &findData));
            FindClose(hFind);
        }
        _rmdir(path.c_str());
...
public:
    ...
    void viewFiles() {
        ...
        for (const auto& name : fs::get_files("Vault")) {
            cout << GREEN << "  >> " << name << " (" << fs::file_size("Vault/" + name) << " bytes)\n" << RESET;
        }
    }
    ...
    void searchContent() {
        ...
        bool found = false;
        for (const auto& name : fs::get_files("Vault")) {
            ifstream f("Vault/" + name); string line; int num = 1;
            while (getline(f, line)) {
                if (line.find(keyword) != string::npos) {
                    cout << "  [" << name << "] Line " << num << ": " << line << "\n";
                    found = true;
                }
                num++;
            }
    ...
    void verifyIntegrity() {
        ...
        for (const auto& name : fs::get_files("Vault")) {
            ...
            else if (fs::file_size("Vault/" + name) != fs::file_size(backup)) cout << RED << "  [BREACHED]  " << RESET << name << "\n";
    ...
    void addOrUpdateFile(int mode) {
        ...
        fs::copy_file(path, ".backups/" + name);
    ...
    void restoreSystem() {
        for (auto& name : fs::get_files(".backups"))
            fs::copy_file(".backups/" + name, "Vault/" + name);
    ...
    void securityInfo() {
        ...
        int files = fs::get_files("Vault").size();
